...
function showEntry($t, $metadata, $favourite = FALSE, $withIcon = FALSE) {
  ...
  if($withIcon) {
    ...
    $html .= '<img alt="Identity Provider" class="entryicon" src="' . SimpleSAML_Module::getModuleURL('themeeosc/resources/images/' . $filename ) . '" />';
...
?>
  ...
  <h2 class="text-center"><?php echo (strpos($this->t('{themeeosc:discopower:title}'), 'not translated') === FALSE ? $this->t('{themeeosc:discopower:title}') : ''); ?></h2>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h2 class="modal-title">' . (strpos($this->t('{themeeosc:discopower:favourite_dialog_title}'), 'not translated') === FALSE ? $this->t('{themeeosc:discopower:favourite_dialog_title}') : '') . '</h2>
          </div>
          <div class="modal-body ssp-modal-body">
            ...
            <div class="row text-center">
              <button class="ssp-btn text-uppercase ssp-btn ssp-btn__secondary js-close-custom">' . (strpos($this->t('{themeeosc:discopower:favourite_dialog_button_close}'), 'not translated') === FALSE ? $this->t('{themeeosc:discopower:favourite_dialog_button_close}') : '') . '</button>
            </div>
          </div> <!-- /modal-body -->
        </div> <!-- /modal-content -->
      </div> <!-- /modal-dialog -->
  ...
  if (!empty($slist)) {
    if($tab == 'idps_in_searchable_list') {
      ...
      if($idps_in_searchable_list_index < $idps_with_logos_index) {
        ...
        $or = '<div class="text-center ssp-line-or-line ssp-line-or-line--bottom"><span class="ssp-line-or-line__or">' . (strpos($this->t('{themeeosc:discopower:or}'), 'not translated') === FALSE ? $this->t('{themeeosc:discopower:or}') : '') . '</span></div>';
